// Extracts a medially centered curve skeleton
...
{
    std::ifstream input((argc > 1) ? argv[1] : "data/m16_2_10_2_34_R.off");
    Polyhedron tmesh;
    input >> tmesh;
    ...
    std::ofstream output("skel-poly-cgal"); // m16_2_10_2_34_R.
    Display_polylines display(skeleton, output);
    CGAL::split_graph_into_polylines(skeleton, display);
    //**std::cout << boost::num_vertices(skeleton) << "\n";
    output.close();
    // Output skeleton points and the corresponding surface points
    //* output.open("correspondance-poly-m16_2_10_2_34_R.cgal");
    for (Skeleton_vertex v : CGAL::make_range(vertices(skeleton)))
        for (vertex_descriptor vd : skeleton[v].vertices)
            std::cout << "2 " << skeleton[v].point << " "

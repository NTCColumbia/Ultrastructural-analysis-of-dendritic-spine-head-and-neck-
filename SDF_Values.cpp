...
int main(int argc, char** argv)
{
    ...
    if (argc == 2) {
        ...
        std::ifstream cactus("data/2_R.off"); //full_path);  // 
        cactus >> mesh;
    }
    ...
    // the following lines:
    const std::size_t number_of_rays = 30;  // cast 25 rays per face
    const double cone_angle = 2.0 / 6.0 * CGAL_PI; // set cone opening-angle
